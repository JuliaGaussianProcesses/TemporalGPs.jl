...
@testset "linear_gaussian_dynamics" begin
    ...
    @testset "LGC (Dlat=$Dlat, Dobs=$Dobs, $(storage.name))" for
        Dlat in Dlats,
        Dobs in Dobss,
        storage in storages
        ...
        model = random_lgc(rng, Dlat, Dobs, storage.val)
    ...
    @testset "LargeOutputLGC (Dlat=$Dlat, Dobs=$Dobs, $(storage.name))" for
        ...
        @testset "consistency with LGC" begin
            vanilla_model = TemporalGPs.LGC(model.A, model.a, model.Q)
    ...
    IMPLEMENT THESE TESTS PROPERLY IN THE MORNING!
