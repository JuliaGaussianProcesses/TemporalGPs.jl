...
function rand_tangent(rng::AbstractRNG, d::T) where {T<:Gaussian}
    return Tangent{T}(
...
function rand_tangent(rng::AbstractRNG, gmm::T) where {T<:GaussMarkovModel}
    return Tangent{T}(
        ordering = nothing,
...
function rand_tangent(rng::AbstractRNG, ssm::T) where {T<:LGSSM}
    Hs = ssm.emissions.A
    hs = ssm.emissions.a
    Σs = ssm.emissions.Q
    return Tangent{T}(
